...
def hexadecapole_plots(map_size=a.map_size,sep=a.sep,FWHM=a.FWHM,noise_power=a.noise_power,\
    ...
    for i in range(len(goodMaps)):
        ...
        if a.true_lensing:
            root='LensedPaddedBatchData/'
        else:
            root = 'PaddedBatchData/'
        data=np.load(a.root_dir+root+'f%s_ms%s_s%s_fw%s_np%s_d%s/%s.npy' %(freq,map_size,sep,FWHM,noise_power,delensing_fraction,i))
