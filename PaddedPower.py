...
def DegradeMap(liteMap,lCut):
	...
	fPadOld=fftTools.fftFromLiteMap(liteMap)
	fPad=fPadOld.trimAtL(lCut)
	factor=fPadOld.pixScaleX*fPadOld.pixScaleY/(fPad.pixScaleX*fPad.pixScaleY) # ratio of areas in ell-space
	fPad.kMap*=factor
...
def DegradeFourier(fMap,lCut):
	"""Remove unnecessary high-ell data from Fourier transform map, keeping the same power in the remaining pixels."""
	fPadOld=fMap.copy()
	fPad=fPadOld.trimAtL(lCut) # cut out high ell pixels
	factor=fPadOld.pixScaleX*fPadOld.pixScaleY/(fPad.pixScaleX*fPad.pixScaleY) # rescale to keep power consistent
	fPad.kMap*=factor
	return fPad
	
def fourier_noise_test(padded_mask,unpadded_mask,ell,CellNoise,padding_ratio=a.padding_ratio,unpadded=False,log=False):
	""" See fourier_noise_map for header info."""
	from .RandomField import real_fill_from_Cell
	initial_map,kMap=real_fill_from_Cell(unpadded_mask.copy(),ell,CellNoise,bufferFactor=1,lMin=min(ell),log=log,fourier=True)
	from .PaddedPower import zero_padding
	padded_mask=zero_padding(unpadded_mask.copy(),padding_ratio) # to avoid small length differences
	...
	fUnpadded.kMap=kMap
	del initial_map,padded_mask,unpadded_mask,padded_map,kMap
	#fUnpadded.kMap/=np.sqrt(np.mean(unpadded_mask.data**2.))
	return fPadded,fUnpadded
    
def fourier_noise_map(padded_mask,unpadded_mask,ell,CellNoise,padding_ratio=a.padding_ratio,unpadded=False,log=False):
	...
	log -> use logarithmic interpolation when generating the ell-Cell function
	"""	
	# First compute a large real space map based on the power spectrum
	if unpadded:
		from .RandomField import real_fill_from_Cell
		noise_map=real_fill_from_Cell(padded_mask.copy(),ell,CellNoise,bufferFactor=1,lMin=min(ell),log=log)
		...
		from .RandomField import real_fill_from_Cell
		noise_map=real_fill_from_Cell(padded_mask.copy(),ell,CellNoise,bufferFactor=1,lMin=min(ell),log=log) # compute in linear space
