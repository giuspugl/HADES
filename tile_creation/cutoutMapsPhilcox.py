...
if not os.path.exists(p['workDir']):
...
if doAll:
    ...
    if p['cutoutMask']:
        mask, maskHeaderMask = healpy.fitsfunc.read_map(p['maskDir'], h = True)
    print 'done'
...
mapDecs = [[decVals[i]] * int(n) for i,n in enumerate(nPoints)]
