...
def single_MC(tempMap,model_power,windowFactor=None):
	...
	windowFactor -> <W^2> for window - must be multiplied by to avoid window errors
	
	Output -> MC random map as a two-dimensional power-map
	"""
	#Testing
	if windowFactor==None:
		raise Exception('No window function')
	...
	for i in range(len(data)):
		for j in range(len(data[i])):
			...
			if l>0.5*a.lMin and l<1.5*a.lMax: # if in correct range
				...
				data[i,j] = windowFactor*(amplitude**2.0) # (real by construction)
...
def noise_map(realMap=None,powMap=None,lMin=a.lMin,lMax=a.lMax,FWHM=a.FWHM,noise_power=a.noise_power,windowFactor=None):
	""" Create a Gaussian random noise power map from a given noise profile.
	We multiply by <W^2> to include window function effects.
	...
	powerMap.powerMap=data*windowFactor
