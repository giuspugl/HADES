...
if __name__=='__main__':
	...
	if not a.remakeErrors:
		LEN=create_params()
		
		if index<LEN:
			delensing_fraction=1.0
		...
		errFiles=np.load(a.root_dir+'ErrorFiles_f%s_d%s.npz.npy' %(a.freq,delensing_fraction))
		LEN=len(errFiles)
		index=errFiles[index] # use new indexing
...
def reconstruct_epsilon(map_size=a.map_size,sep=a.sep,freq=a.freq,delensing_fraction=a.delensing_fraction):
	...
	# Save output
	np.save(a.root_dir+'ErrorFiles_f%s_d%s.npz' %(freq,delensing_fraction),errFiles)
...
def noise_params_plot(map_size=a.map_size,sep=a.sep,\
	delensing_fraction=a.delensing_fraction,freq=a.freq,createData=True,S=110):
	...
	plt.title('Significance of Patch Epsilon Detection')
	plt.xlim(XLIM)
	plt.ylim(YLIM)
	# Save plot
