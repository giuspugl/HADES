...
def runner(map_id):
	...
	powB=MakePowerAndFourierMaps(map_id,fourier=False,power=True)
	filt=np.where((powB.modLMap.ravel()<230.)&(powB.modLMap.ravel()>170.))
	
	logL=-1.*np.log(powB.modLMap.ravel()[filt]/200.)
	...
	idx=np.where((powB.modLMap.ravel()<220.)&(powB.modLMap.ravel()>180.))	
...
if __name__=='__main__':
	...
	np.savez(a.root_dir+'Pow200.npz',regression=pow80_regress,single=pow85)
	print 'complete'
