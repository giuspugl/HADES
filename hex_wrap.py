...
def hex_fullsky_stats(map_size=a.map_size,sep=a.sep,FWHM=a.FWHM,noise_power=a.noise_power,\
	...
	for i in range(len(goodMaps)):
		...
		data=np.load(a.root_dir+'HexBatchData/f%s_ms%s_s%s_fw%s_np%s_d%s/%s.npy' %(freq,map_size,sep,FWHM,noise_power,delensing_fraction,map_id))
...
def patch_hexadecapole(map_size=a.map_size,sep=a.sep,FWHM=a.FWHM,noise_power=a.noise_power,
	freq=a.freq,delensing_fraction=a.delensing_fraction,N_sims=a.N_sims,suffix='',folder=None,root_dir=a.root_dir,I2SNR=a.I2SNR,returnAll=False,plot=True):
	...
	for i in range(len(goodMaps)):
		...
		if I2SNR:
			SNR=1.#A/A_est#(hex2_mean/hex2_eps)**2.#(data[0][1]/data[0][2])**2.#/hex_eps**2.#I2data[i]**2./hex_eps**2.
		else:
