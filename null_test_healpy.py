...
if __name__=='__main__':
	...
	outDir=a.root_dir+'HealpyFullMapNullTestData/f%s_ms%s_s%s_fw%s_np%s_d%s/' %(a.freq,a.map_size,a.sep,a.FWHM,a.noise_power,a.delensing_fraction)
	...
	from hades.healpy_wrap2 import wrapper
	def runner(map_id): return wrapper(map_id,f_dust=f_dust)
	...
	for j in range(err_repeats): # repeat for MC errors
		print 'f_dust %s tile %s healpy run %s starting for map_id %s' %(f_dust,batch_id,j,map_id)		
...
def create_significances(map_size=a.map_size,sep=a.sep,FWHM=a.FWHM,noise_power=a.noise_power,delensing_fraction=a.delensing_fraction,freq=a.freq,root_dir=a.root_dir,\
	...
	for i in range(len(a.f_dust_all)):
		...
		for j in range(a.err_repeats):
			...
			A.append(outs[2])
		mean[i]=np.mean(sigs)
		monopole[i]=np.mean(A)
		true_monopole[i]=np.mean(trueA)
		
		if a.err_repeats!=1:
			err[i]=np.std(sigs)
			print 'f_dust: %s' %a.f_dust_all[i] #mean: %s std: %s' %(a.f_dust_all[i],mean[i],err[i])
		else:
	...
	if plot:
		...
		plt.savefig(outDir+'HealpyNullTestPaddedPlot_f%s_ms%s_s%s_fw%s_np%s_d%s.png' %(a.freq,a.map_size,a.sep,a.FWHM,a.noise_power,a.delensing_fraction),bbox_inches='tight')
		plt.close()
	
	outDir=root_dir+'HealpyFullMapNullTests/'
	import os
...
def plot_hexadecapole(map_size=a.map_size,sep=a.sep,FWHM=a.FWHM,noise_power=a.noise_power,delensing_fraction=a.delensing_fraction,freq=a.freq,root_dir=a.root_dir):
	...
	from hades.null_test_healpy import patch_hexadecapole
	h2_mean,h2_est,h2_eps,bias=[],[],[],[]
	for i in range(len(a.f_dust_all)):
		suffix='_'+str(i)+'_0'#+str(0)
		outs=patch_hexadecapole(suffix=suffix,map_size=map_size,sep=sep,FWHM=FWHM,noise_power=noise_power,\
				delensing_fraction=delensing_fraction,freq=freq,folder='HealpyNullTestBatchData',returnAll=True)
	...
	#plt.yscale('log')
	plt.legend()
	plt.title(r'Healpy Null Test Hexadecapole Strength')
	outDir=root_dir+'HealpyNullTests/'
	import os
	if not os.path.exists(outDir):
		os.makedirs(outDir)
	plt.savefig(outDir+'HealpyPaddedH2Plot_f%s_ms%s_s%s_fw%s_np%s_d%s.png' %(a.freq,a.map_size,a.sep,a.FWHM,a.noise_power,a.delensing_fraction),bbox_inches='tight')
	plt.close()
