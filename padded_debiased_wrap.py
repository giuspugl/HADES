...
def padded_wrap(map_id,map_size=a.map_size,\
	...
	rot_average=a.rot_average,useBias=a.useBias,padding_ratio=a.padding_ratio):
	...
	"""
	lCut=2.*lMax # maximum ell for Fourier space maps
	...
	from .PaddedPower import MakePowerAndFourierMaps,DegradeMap
	...
	unpadded_fBdust=unpadded_fBdust.trimAtL(lCut)
	
	fBdust=fBdust.trimAtL(lCut) # discard high-ell pixels
	...
	if a.hexTest:
		...
		ll,pp=oneD_binning(powDust.copy(),0,3000,l_step,binErr=False,exactCen=False) # compute one-D binned spectrum
	...
	if useBias:
		...
		for n in range(N_bias):
			...
			fBias=padded_fill_from_Cell(padded_window.copy(),l_cen,mean_pow,lMin=lMin)#,padding_ratio=padding_ratio)
			#powBias=fftTools.powerFromFFT(fBias.copy())
			#all_sims.append(powBias)
			#return all_sims,l_cen,mean_pow,totPow,padded_window
			#if True:
			#if True:
			#return all_sims,l_cen,mean_pow
			#if True:
			#	if True:
			#fBias=fBias.trimAtL(1.5*lMax) # discard high-ell pixels
			bias_cross=fftTools.powerFromFFT(fBias.copy(),totFmap.copy()) # cross map
			bias_self=fftTools.powerFromFFT(fBias.copy()) # self map
